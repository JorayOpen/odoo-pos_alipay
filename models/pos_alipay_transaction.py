...
class AlipayTransaction(models.Model):
    _name = 'pos_alipay.alipay_transaction'
    ...
    def _get_pos_alipay_config_id(self, config, journal_id):
        journal = config.journal_ids.filtered(lambda r: r.id == journal_id)

        if journal and journal.pos_alipay_config_id:
            return journal.pos_alipay_config_id
        else:
            raise UserError("No Alipay configuration associated with the journal.")

    def _setup_request(self, data):
        pos_session = self._get_pos_session()

        config = pos_session.config_id
        pos_alipay_config = self._get_pos_alipay_config_id(config, data['journal_id'])

        data['operator_id'] = pos_session.user_id.login
        data['merchant_id'] = pos_alipay_config.sudo().merchant_id
        data['merchant_pwd'] = pos_alipay_config.sudo().merchant_pwd
    ...
    def _do_request(self, template, data):
        ...
        soap_header = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:mer="http://www.alipaypay.com"><soapenv:Header/><soapenv:Body><mer:CreditTransaction><mer:tran>'
        ...
        headers = {
            ...
            'SOAPAction': 'http://www.alipaypay.com/CreditTransaction',
        }

        r = urllib2.Request('https://w1.alipaypay.com/ws/ws.asmx', data=xml_transaction, headers=headers)
        try:
    ...
    def _do_reversal_or_voidsale(self, data, is_voidsale):
        ...
        response = self._do_request('pos_alipay.alipay_voidsale', data)
        return response
    ...
    def do_payment(self, data):
        ...
        response = self._do_request('pos_alipay.alipay_transaction', data)
        return response
    ...
    def do_return(self, data):
        ...
        response = self._do_request('pos_alipay.alipay_return', data)
        return response

    # One time (the ones we use) Alipay tokens are required to be
