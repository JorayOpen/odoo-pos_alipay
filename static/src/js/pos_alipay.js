odoo.define('pos_alipay.pos_alipay', function (require) {
"use strict";
...
pos_model.load_fields("account.journal", "pos_alipay_config_id");
...
pos_model.PosModel = pos_model.PosModel.extend({
    ...
    decodeAlipayResponse: function (data) {
...
pos_model.Paymentline = pos_model.Paymentline.extend({
    init_from_JSON: function (json) {
        ...
        this.paid = json.paid;
        this.alipay_card_number = json.alipay_card_number;
        this.alipay_card_brand = json.alipay_card_brand;
        this.alipay_card_owner_name = json.alipay_card_owner_name;
        this.alipay_ref_no = json.alipay_ref_no;
        this.alipay_record_no = json.alipay_record_no;
        this.alipay_invoice_no = json.alipay_invoice_no;
        this.alipay_auth_code = json.alipay_auth_code;
        this.alipay_data = json.alipay_data;
        this.alipay_swipe_pending = json.alipay_swipe_pending;

        this.set_credit_card_name();
    },
    export_as_JSON: function () {
        return _.extend(_paylineproto.export_as_JSON.apply(this, arguments), {paid: this.paid,
                                                                              alipay_card_number: this.alipay_card_number,
                                                                              alipay_card_brand: this.alipay_card_brand,
                                                                              alipay_card_owner_name: this.alipay_card_owner_name,
                                                                              alipay_ref_no: this.alipay_ref_no,
                                                                              alipay_record_no: this.alipay_record_no,
                                                                              alipay_invoice_no: this.alipay_invoice_no,
                                                                              alipay_auth_code: this.alipay_auth_code,
                                                                              alipay_data: this.alipay_data,
                                                                              alipay_swipe_pending: this.alipay_swipe_pending});
    },
    set_credit_card_name: function () {
        if (this.alipay_card_number) {
            this.name = this.alipay_card_brand + " (****" + this.alipay_card_number + ")";
        }
    }
});
...
PaymentScreenWidget.include({
    ...
    // a Alipay transaction
    server_timeout_in_ms: 95000,

    // How many Alipay transactions we send without receiving a
    // response
    server_retries: 3,
    ...
    _get_swipe_pending_line: function () {
        ...
        for (i = 0; i < lines.length; i++) {
            if (lines[i].alipay_swipe_pending) {
                return lines[i];
            }
        }
    ...
    _does_credit_payment_line_exist: function (amount, card_number, card_brand, card_owner_name) {
        ...
        for (i = 0; i < lines.length; i++) {
            if (lines[i].alipay_amount === amount &&
                lines[i].alipay_card_number === card_number &&
                lines[i].alipay_card_brand === card_brand &&
                lines[i].alipay_card_owner_name === card_owner_name) {
                return true;
            }
        }
    ...
    retry_alipay_transaction: function (def, response, retry_nr, can_connect_to_server, callback, args) {
        var self = this;
        var message = "";
        ...
        if (retry_nr < self.server_retries) {
            ...
            if (response) {
                ...
                if (can_connect_to_server) {
                    message = _t("No response from Alipay (Alipay down?)");
                } else {
    ...
    credit_code_transaction: function (parsed_result, old_deferred, retry_nr) {
        ...
        var alipay_transaction = new Model('pos_alipay.alipay_transaction');
        alipay_transaction.call('do_payment', [transaction], undefined, {timeout: self.server_timeout_in_ms}).then(function (data) {
            // if not receiving a response from Alipay, we should retry
            if (data === "timeout") {
                self.retry_alipay_transaction(def, null, retry_nr, true, self.credit_code_transaction, [parsed_result, def, retry_nr + 1]);
                return;
            }

            if (data === "not setup") {
                def.resolve({
                    message: _t("Please setup your Alipay merchant account.")
                });
                return;
            }
            ...
            var response = self.pos.decodeAlipayResponse(data);
            ...
            if (response.status === 'Approved') {
                ...
                if (response.message === "AP*" && self._does_credit_payment_line_exist(response.authorize, decodedMagtek['number'],
                    ...
                    order.selected_paymentline.alipay_swipe_pending = false;
                    order.selected_paymentline.alipay_amount = response.authorize;
                    order.selected_paymentline.alipay_card_number = decodedMagtek['number'];
                    order.selected_paymentline.alipay_card_brand = response.card_type;
                    order.selected_paymentline.alipay_card_owner_name = decodedMagtek['name'];
                    order.selected_paymentline.alipay_ref_no = response.ref_no;
                    order.selected_paymentline.alipay_record_no = response.record_no;
                    order.selected_paymentline.alipay_invoice_no = response.invoice_no;
                    order.selected_paymentline.alipay_auth_code = response.auth_code;
                    order.selected_paymentline.alipay_data = response; // used to reverse transactions
            ...
            // if an error related to timeout or connectivity issues arised, then retry the same transaction
                ...
                if (lookUpCodeTransaction["TimeoutError"][response.error]) { // recoverable error
                    self.retry_alipay_transaction(def, response, retry_nr, true, self.credit_code_transaction, [parsed_result, def, retry_nr + 1]);
            ...
            self.retry_alipay_transaction(def, null, retry_nr, false, self.credit_code_transaction, [parsed_result, def, retry_nr + 1]);
        });
    },
    ...
    credit_code_action: function (parsed_result) {
        ...
        } else { // this is for supporting another payment system like alipay
    ...
    do_reversal: function (line, is_voidsale, old_deferred, retry_nr) {
        ...
        var alipay_transaction = new Model('pos_alipay.alipay_transaction');
        alipay_transaction.call(rpc_method, [request_data], undefined, {timeout: self.server_timeout_in_ms}).then(function (data) {
            if (data === "timeout") {
                self.retry_alipay_transaction(def, null, retry_nr, true, self.do_reversal, [line, is_voidsale, def, retry_nr + 1]);
                return;
            }
            ...
            event.preventDefault();
            self.retry_alipay_transaction(def, null, retry_nr, false, self.do_reversal, [line, is_voidsale, def, retry_nr + 1]);
        });
    },
    ...
    click_delete_paymentline: function (cid) {
        ...
        for (var i = 0; i < lines.length; i++) {
            if (lines[i].cid === cid && lines[i].alipay_data) {
    ...
    click_paymentmethods: function (id) {
        ...
        if (cashregister.journal.pos_alipay_config_id) {
            var already_swipe_pending = false;
            var lines = order.get_paymentlines();

            for (i = 0; i < lines.length; i++) {
                if (lines[i].cashregister.journal.pos_alipay_config_id && lines[i].alipay_swipe_pending) {
            ...
            if (already_swipe_pending) {
                ...
                this._super(id);
                order.selected_paymentline.alipay_swipe_pending = true;
                this.render_paymentlines();
                order.trigger('change', order); // needed so that export_to_JSON gets triggered
            }
        } else {
